...
- **Shareable proposal** — **Open proposal** on the calculator opens [`?proposal=sow`](https://pixellayer7-jpg.github.io/project-estimator/?proposal=sow) with the **same price, scope, quote ID, and timeline dates**. Clients **type their name to accept**, print/PDF, then land on the matching portal. Optional **client name** appears on the proposal, invoice, and portal.
- **Kickoff on the portal** — After accept, clients can **Mark deposit sent** and check off assets / copy / access. Completing the checklist hands next action to PixelLayer. Stored in this browser only — no Stripe.
